...
// Here we implement linear ordering of vertics of DAG such that for every directed edge a and b, vertex a comes before b in the order using modified DFS.
...
// Code Begins 
vector <long int> adj[100001];//Vector which store the adjacency list
...
{
    vis[v]=true;// assign the edge visited 
    for(long long int i = 0;i<adj[v].size();i++){
        if(!vis[adj[v][i]])// Every unvisited child of the edge is visited recursively 
        toposort(adj[v][i]);
    }

    order.push(v);//Required edge is pushed in order
}
 
int main(){

	long int n,m;// No of vertics and edges respeectively
	cin>>n>>m;
